...
def find_closest_num(A, target):
    ...
    # Edge cases for empty list or when the list is only one elements.
    if len(A) == 0:
        return None
    if len(A) == 1:
        return A[0]
    ...
    while low <= high:
        ...
        # If the elements is the target itself, the closest number to it is itself  
        else:
            return A[mid]
...
x = find_closest_num(A, targetA)
y = find_closest_num(B, targetB)
